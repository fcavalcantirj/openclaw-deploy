...
echo -e "${MAGENTA}${BOLD}║                                                                   ║${NC}"
echo -e "${MAGENTA}${BOLD}║   🚀  OPENCLAW-DEPLOY - RALPH CONTINUOUS RUNNER                             ║${NC}"
echo -e "${MAGENTA}${BOLD}║                                                                   ║${NC}"
...
while true; do
  ...
  send_telegram "🚀 *openclaw-deploy Ralph* - Batch #${batch_count} Starting
...
🔄 Running ${BATCH_SIZE} iterations..."
  ...
  if [ "$api_error" = true ]; then
    ...
    if [ -n "$error_detail" ]; then
      send_telegram "🚨 *openclaw-deploy Ralph* - API Error
...
📊 Progress: $(./progress.sh)"
    ...
    send_telegram "✅ *openclaw-deploy Ralph* - Batch #${batch_count} Complete
