...
while true; do
    ...
    if [ "$api_error" = true ]; then
        # Cross-platform date: try GNU first, fall back to macOS
        resume_time=$(date -d "+${WAIT_TIME_HOURS} hours" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || \
                      date -v+${WAIT_TIME_HOURS}H '+%Y-%m-%d %H:%M:%S')
        ...
        # Cross-platform date
        resume_time=$(date -d "+${BATCH_PAUSE_MINS} minutes" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || \
                      date -v+${BATCH_PAUSE_MINS}M '+%Y-%m-%d %H:%M:%S')
